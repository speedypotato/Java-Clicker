 ...
 */
public class EditorPanel extends JComponent implements Runnable, Serializable {
	...
	public EditorPanel(String name) {
		...
		addMouseListener(new MouseAdapter() { //repaint when mouse clicks
			public void mousePressed(MouseEvent e) {
				double offset = (getCircleSize() * getBounds().width) / 2;
				double xPerc = ((double)e.getX() - offset) / (double)getWidth();
				double yPerc = ((double)e.getY() - offset) / (double)getHeight();
	 ...
	 * @return The amount of time for the approach circle to reach the hit circle in ms
	 */
	...
	private transient final File songFile;
	...
	private transient ArrayList<HitObject> hitObjects;
	private transient ArrayList<HitObject> currentlyDisplayed;
}
